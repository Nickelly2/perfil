# 👋 Olá, eu sou a Nickelly!

### 📌 Sobre mim

Sou estudante de **Análise e Desenvolvimento de Sistemas**, apaixonada por **Python** e iniciando minha jornada no universo da programação para dados. Tenho experiência mínima, mas estou 100% focada em transformar teoria em prática, desenvolvendo projetos reais e adquirindo experiência para crescer na área de tecnologia.

> *"Estudante de ADS | Explorando Python e Análise de Dados. Buscando minha 1ª oportunidade para aplicar conhecimentos, adquirir prática e crescer na área de T.I."*

---

### 🛠️ Tecnologias em desenvolvimento

- **🐍 Python** (Pandas, NumPy - básico/intermediário)
- **🗄️ SQL** (Consultas e manipulação de dados)
- **🌐 Web básico** (HTML, CSS)
- **⚙️ Ferramentas**: Git, GitHub e VS Code

---

### 📊 O que estou fazendo agora

- Aprofundando meus estudos em **Python para Análise de Dados**.
- Criando pequenos projetos de portfólio para demonstrar minhas habilidades.
- Buscando ativamente minha **primeira oportunidade** como desenvolvedora ou analista de dados júnior.

---

### 📫 Como me encontrar

- [LinkedIn] (www.linkedin.com/in/nickelly-rackelly-da-luz-eleuterio-a32912371)
- [E-mail] (nickeleuterio08@gmail.com)
